...
public class WebSocketServerDecoder {
  ...
  public static WebSocketRequest decode(ByteBuffer buf, ChannelContext channelContext) throws TioDecodeException {
    ...
    if (payloadLength == 126) { // 为126读2个字节，后两个字节为payloadLength
      ...
      // log.info("{} payloadLengthFlag: 126，payloadLength {}", channelContext,
      // payloadLength);
      ...
      // log.info("{} payloadLengthFlag: 127，payloadLength {}", channelContext,
      // payloadLength);
    }
    ...
    // if (!fin) {
    // //lastParts.add(array);
    // log.error("payloadLength {}, lastParts size {}, array length {}",
    // payloadLength,

...
/**
 * Default implementation of Paranamer reads from a post-compile added field
 * called '__PARANAMER_DATA'
 *
 * @author Paul Hammant
 * @author Mauro Talevi
...
public class DefaultParanamer implements Paranamer {
  ...
  private static String getParameterTypeName(Class<?> cls) {
    ...
    parameterTypeNameName = parameterTypeNameName.replace("[J", "[Xlong").replace("[I", "[Xint").replace("[Z", "[Xboolean").replace("[S", "[Xshort")
        .replace("[F", "[Xfloat").replace("[D", "[Xdouble").replace("[B", "[Xbyte").replace("[C", "[Xchar");

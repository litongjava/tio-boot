...
import nexus.io.tio.http.common.ResponseLine;
...
public class HttpResponseDecoder {
  ...
  public static HttpResponse decode(ByteBuffer buffer, int limit, int position, int readableLength, ChannelContext channelContext) throws TioDecodeException {
    ...
    String contentLengthStr = headers.get(ResponseHeaderKey.Content_Length);
    ...
    httpResponse.addHeaders(headers.entrySet().stream().collect(Collectors.toMap(entry -> HeaderName.from(entry.getKey()), entry -> HeaderValue.from(entry.getValue()))));
//    httpResponse.setContentLength(contentLength);
    String connection = headers.get(ResponseHeaderKey.Connection);
    if (connection != null) {
//      httpResponse.setConnection(connection.toLowerCase());
    ...
    if (contentLength == 0) {
      ...
      // decodeParams(httpResponse.getParams(), firstLine.getQuery(),

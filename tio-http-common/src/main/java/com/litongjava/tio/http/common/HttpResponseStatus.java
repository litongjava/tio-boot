...
public enum HttpResponseStatus {
   ...
   * Temporatily”。出现该状态代码时，浏览器能够自动访问新的URL，因此它是一个很有用的状态代码。注意这个状态代码有时候可以和301替换使用。例如，如果浏览器错误地请求http://host/~user（缺少了后面的斜杠），有的服务器
   * 返回301，有的则返回302。严格地说，我们只能假定只有当原来的请求是GET时浏览器才会自动重定向。请参见307。
   */
  ...
  /**
   * 客户端有缓冲的文档并发出了一个条件性的请求（一般是提供If-Modified-Since头表示客户只想比指定日期更新的文档）。服务器告
   * 诉客户，原来缓冲的文档还可以继续使用。
   */
  ...
  /**
   * 和302 （Found）相同。许多浏览器会错误地响应302应答进行重定向，即使原来的请求是POST，即使它实际上只能在POST请求的应答是303时才能重定
   * 向。由于这个原因，HTTP
  ...
  /**
   * 客户试图未经授权访问受密码保护的页面。应答中会包含一个WWW-Authenticate头，浏览器据此显示用户名字/密码对话框，然后在填
  ...
  /**
   * 所请求的文档已经不再可用，而且服务器不知道应该重定向到哪一个地址。它和404的不同在于，返回407表示文档永久地离开了指定的位置，而
   * 404表示由于未知的原因文档不可用。（HTTP 1.1新）
   */
  ...
  /**
   * 服务器由于维护或者负载过重未能应答。例如，Servlet可能在数据库连接池已满的情况下返回503。服务器返回503时可以提供一个
   * Retry-After头。
   */
  ...
  /**
   */
  CUSTOM(0, "Custom Status", "0 Custom Status");
  ...
  HttpResponseStatus(String version, int status, String description, String headerText) {
    build(version, status, description, headerText);
  }

  public HttpResponseStatus buildHttp10(int status, String description, String headerText) {
    return build("1.0", status, description, headerText);
  }
  ...
  HttpResponseStatus build(String version, int status) {
    String headerText = status + " " + description;
    this.status = status;
    this.headerText = headerText;
    this.headerBinary = headerText.getBytes();
  ...
  public HttpResponseStatus build(String version, int status, String description, String headerText) {
    this.status = status;
    this.description = description;
    this.headerText = headerText;
    this.headerBinary = headerText.getBytes();
  ...
  public HttpResponseStatus build(int status, String description) {
    String headerText = status + " " + description;
    this.status = status;
    this.description = description;
    this.headerText = headerText;
  ...
  public HttpResponseStatus build(int status) {
    this.status = status;
    this.description = "Unknow";
    this.headerText = status + " Unknow ";
